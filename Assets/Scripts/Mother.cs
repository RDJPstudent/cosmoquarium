...
public class Mother : Fish
{
    ...
    // Finds every non-predator Fish currently in the scene and picks one at random.
    // Excludes self and anything flagged isPredator (Alien, Bug, or Mother), so predators never target each other.
    protected virtual void AcquireRandomTarget()
    {
        ...
        foreach (Fish fish in allFish)
        {
            ...
            if (fish.isPredator) continue; // excludes ANY predator-flagged Fish, regardless of which script it uses
